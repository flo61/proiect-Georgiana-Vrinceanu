// <auto-generated />
using System;
using Geo.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Geo.Migrations
{
    [DbContext(typeof(GeoContext))]
    [Migration("20230119210140_nine")]
    partial class nine
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.13")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("Geo.Models.Carte", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<string>("DenumireCarte")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.ToTable("Carte");
                });

            modelBuilder.Entity("Geo.Models.Countriestovisit", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<string>("CountryName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.ToTable("Countriestovisit");
                });

            modelBuilder.Entity("Geo.Models.Incredere", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<DateTime>("ReturnDate")
                        .HasColumnType("datetime2");

                    b.Property<int?>("TodolistID")
                        .HasColumnType("int");

                    b.Property<int?>("WhocanseeID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("TodolistID");

                    b.HasIndex("WhocanseeID");

                    b.ToTable("Incredere");
                });

            modelBuilder.Entity("Geo.Models.Materie", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<string>("Materiipecaresaleiau")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.ToTable("Materie");
                });

            modelBuilder.Entity("Geo.Models.Obiective", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<string>("ObiectivulMeu")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.ToTable("Obiective");
                });

            modelBuilder.Entity("Geo.Models.Todolist", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<int?>("CarteID")
                        .HasColumnType("int");

                    b.Property<string>("Countries")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CountriestovisitID")
                        .HasColumnType("int");

                    b.Property<string>("Goals")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MaterieID")
                        .HasColumnType("int");

                    b.Property<string>("Movies")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ObiectiveID")
                        .HasColumnType("int");

                    b.Property<decimal>("Rude")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("Thedate")
                        .HasColumnType("datetime2");

                    b.HasKey("ID");

                    b.HasIndex("CarteID");

                    b.HasIndex("CountriestovisitID");

                    b.HasIndex("MaterieID");

                    b.HasIndex("ObiectiveID");

                    b.ToTable("Todolist");
                });

            modelBuilder.Entity("Geo.Models.Whocansee", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<string>("Adress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Phone")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.ToTable("Whocansee");
                });

            modelBuilder.Entity("Geo.Models.Incredere", b =>
                {
                    b.HasOne("Geo.Models.Todolist", "Todolist")
                        .WithMany()
                        .HasForeignKey("TodolistID");

                    b.HasOne("Geo.Models.Whocansee", "Whocansee")
                        .WithMany("Trust")
                        .HasForeignKey("WhocanseeID");

                    b.Navigation("Todolist");

                    b.Navigation("Whocansee");
                });

            modelBuilder.Entity("Geo.Models.Todolist", b =>
                {
                    b.HasOne("Geo.Models.Carte", "Carte")
                        .WithMany("Lists")
                        .HasForeignKey("CarteID");

                    b.HasOne("Geo.Models.Countriestovisit", "Countriestovisit")
                        .WithMany("Lists")
                        .HasForeignKey("CountriestovisitID");

                    b.HasOne("Geo.Models.Materie", "Materie")
                        .WithMany("Lists")
                        .HasForeignKey("MaterieID");

                    b.HasOne("Geo.Models.Obiective", "Obiective")
                        .WithMany("Lists")
                        .HasForeignKey("ObiectiveID");

                    b.Navigation("Carte");

                    b.Navigation("Countriestovisit");

                    b.Navigation("Materie");

                    b.Navigation("Obiective");
                });

            modelBuilder.Entity("Geo.Models.Carte", b =>
                {
                    b.Navigation("Lists");
                });

            modelBuilder.Entity("Geo.Models.Countriestovisit", b =>
                {
                    b.Navigation("Lists");
                });

            modelBuilder.Entity("Geo.Models.Materie", b =>
                {
                    b.Navigation("Lists");
                });

            modelBuilder.Entity("Geo.Models.Obiective", b =>
                {
                    b.Navigation("Lists");
                });

            modelBuilder.Entity("Geo.Models.Whocansee", b =>
                {
                    b.Navigation("Trust");
                });
#pragma warning restore 612, 618
        }
    }
}
